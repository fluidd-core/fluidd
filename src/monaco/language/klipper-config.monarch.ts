...
const klipperConfigMonarchLanguage: Monaco.languages.IMonarchLanguage = {
  ...
  knownConfigBlockNames: [
    'ad5206', 'adc_scaled', 'adc_temperature', 'adxl345', 'angle',
    'bed_mesh', 'bed_screws', 'bed_tilt', 'bltouch', 'board_pins',
    'controller_fan',
    'delayed_gcode', 'delta_calibrate', 'display', 'display_data',
    'display_glyph', 'display_status', 'display_template', 'dotstar',
    'dual_carriage', 'duplicate_pin_override',
    'endstop_phase', 'extruder_stepper',
    ...
    'heater_generic', 'homing_heaters', 'homing_override',
    'idle_timeout', 'include', 'input_shaper',
    'led',
    'manual_probe', 'manual_stepper', 'mcp4018', 'mcp4451', 'mcp4728',
    'mcu', 'menu', 'multi_pin',
    'neopixel',
    'output_pin',
    'palette2', 'pause_resume', 'pca9533', 'pca9632', 'printer', 'probe',
    'quad_gantry_level',
    'replicape', 'resonance_tester', 'respond',
    'safe_z_home', 'samd_sercom', 'save_variables',
    'screws_tilt_adjust', 'sdcard_loop', 'servo', 'skew_correction',
    'smart_effector', 'static_digital_output', 'static_pwm_clock',
    'sx1509',
    'temperature_fan', 'temperature_sensor', 'thermistor',
  ...
  gcodeExtendedCommands: [
    'ABORT', 'ACCELEROMETER_DEBUG_READ', 'ACCELEROMETER_DEBUG_WRITE',
    'ACCELEROMETER_MEASURE', 'ACCELEROMETER_QUERY', 'ACCEPT',
    'ACTIVATE_EXTRUDER', 'ANGLE_CALIBRATE', 'ANGLE_DEBUG_READ',
    'ANGLE_DEBUG_WRITE', 'AXIS_TWIST_COMPENSATION_CALIBRATE',
    ...
    'QUERY_ADC', 'QUAD_GANTRY_LEVEL', 'QUERY_ENDSTOPS',
    ...
    'SET_DIGIPOT', 'SET_DISPLAY_GROUP', 'SET_DUAL_CARRIAGE',
    'SET_EXTRUDER_ROTATION_DISTANCE', 'SET_EXTRUDER_STEP_DISTANCE',
  ...
  gcodeExtendedParameters: [
    'AC', 'ACCEL', 'ACCEL_TO_DECEL', 'AD', 'ADAPTIVE', 'ADAPTIVE_MARGIN',
    'ADVANCE', 'ANGLE', 'AXIS',
    'BAND', 'BD', 'BLUE',
    'CARRIAGE', 'CENTER', 'CHIP', 'CHIPS', 'CLEAR', 'COMMAND', 'COUNT',
    'CURRENT', 'CURRENT_LAYER', 'CYCLE_TIME',
    'DAMPING_RATIO_X', 'DAMPING_RATIO_Y', 'DIGIPOT', 'DIRECTION',
    'DISPLAY', 'DISTANCE', 'DURATION',
    'E', 'ENABLE', 'ERROR', 'EXTRUDER',
    'F', 'FACTOR', 'FAN', 'FIELD', 'FILENAME', 'FREQ_END', 'FREQ_START',
    'GREEN', 'GROUP',
    ...
    'S', 'SAMPLE_COUNT', 'SAMPLE_RETRACT_DIST', 'SAMPLES',
    'SAMPLES_RESULT', 'SAMPLES_TOLERANCE', 'SAMPLES_TOLERANCE_RETRIES',
    'SAVE', 'SENSITIVITY', 'SENSOR', 'SERVO', 'SET_POSITION',
    'SHAPER_FREQ_X', 'SHAPER_FREQ_Y', 'SHAPER_TYPE', 'SHAPER_TYPE_X',
    'SHAPER_TYPE_Y', 'SKIP', 'SMOOTH_TIME', 'SPEED',
    'SQUARE_CORNER_VELOCITY', 'START', 'STEP_DELTA', 'STEP_HEIGHT',
    'STEPPER', 'STOP_ON_ENDSTOP', 'SYNC',
    'T', 'TARGET', 'TEMP_COEFF', 'TEMPERATURE_FAN', 'TEMPLATE',
    'TIMEOUT', 'TOTAL_LAYER', 'TRANSMIT', 'TYPE',
  ...
  gcodeExtendedParamValues: [
    '5V', 'average', 'ccw', 'command', 'copy', 'cw', 'echo', 'error',
    'manual', 'median', 'mirror', 'OD', 'output_mode_store',
    'pin_down', 'pin_up', 'primary', 'reset', 'self_test',
    'set_5V_output_mode', 'set_OD_output_mode', 'touch_mode'
  ],

  knownThermistorTypes: /(?:EPCOS 100K B57560G104F|ATC Semitec 104GT-2|ATC Semitec 104NT-4-R025H42G|Generic 3950|NTC 100K beta 3950|Honeywell 100K 135-104LAG-J01|NTC 100K MGB18-104F39050L32|SliceEngineering 450|TDK NTCG104LH104JT1)/i,

  knownSensorTypes: /(?:MAX6675|MAX31855|MAX31856|MAX31865|PT100 INA826|AD595|AD597|AD8494|AD8495|AD8496|AD8497|AHT10|PT1000|BMP280|BME280|BME680|HTU21D|SHT3X|SI7013|SI7020|SI7021|SHT21|LM75|temperature_mcu|temperature_host|DS18B20)/i,

  knownControlTypes: /(?:watermark|pid)/i,

  knownDisplayTypes: /(?:hd44780|st7920|uc1701|ssd1306|sh1106)/i,

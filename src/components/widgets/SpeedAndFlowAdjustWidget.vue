<template>
  <v-row>
    <v-col cols="12" sm="6">
      ...
      <input-slider
        :label="$t('printer.tool.current.xyzMoveSpeed')"
        value-suffix="%"
        input-xs
    ...
    <v-col cols="12" sm="6">
      <input-slider
        :label="$t('printer.tool.current.eFlow')"
        value-suffix="%"
        input-xs
        v-model.number="flow"
...
export default class SpeedAndFlowAdjustWidget extends Mixins(StateMixin) {
  ...
  rules = [
    (v: number) => (v >= 1) || this.$t('app.form.min', { min: 1 }),
    (v: number) => (v <= 200) || this.$t('app.form.max', { max: 200 })
  ]

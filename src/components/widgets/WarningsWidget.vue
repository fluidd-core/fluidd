<template>
  <v-alert text dense icon="$alert" type="warning">
    <div class="text-body-1 mb-2">{{ appName }} {{$t('warnings found.')}}</div>
    ...
    <div v-if="printerWarnings.length > 0">{{$t('Fluidd setup requirements can be found')}} <a target="_blank" :href="docsUrl">here.</a></div>
    <div v-if="moonrakerWarnings.length > 0">{{$t('Moonraker plugin configuration can be found')}} <a target="_blank" :href="moonrakerDocsUrl">here.</a></div>
  </v-alert>
</template>

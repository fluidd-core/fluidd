            ...
            <v-menu bottom left offset-y transition="slide-y-transition" min-width="220" :close-on-content-click="false">
                <template #activator="{ on, attrs, value }">
                    <app-btn small v-bind="attrs" v-on="on" class="me-1 my-1">
                        ...
                        Tools
                        <v-icon small class="ms-1" :class="{ 'rotate-180': value }">
                        $chevronDown
                        </v-icon>
                    </app-btn>
                </template>
                <v-list dense>
                    ...
                    <v-list-item :disabled="!enabled || !canSend"
                                 :class="{ 'mmu-disabled': !enabled }"
                ...
                <v-row align="start">
                    <v-col :cols="col1Size" class="pt-0 d-flex flex-column align-center justify-center">
                        ...
                        <template v-if="showClogDetection">
                            ...
                            <div class="text--disabled">{{ $t('app.mmu.label.clog_detection') }}</div>
                        </template>
                    </v-col>
                    <v-col :cols="12 - col1Size" class="d-flex flex-column align-center justify-center">
                        ...
                        <template v-if="showTtgMap">
                            <mmu-ttg-map
                                ...
                                :selectedGate="gate"
                                @click="showEditTtgMapDialog = true" />
                            <div class="text--disabled">{{ $t('app.mmu.label.tool_mapping') }}</div>
                        </template>
                    </v-col>
                </v-row>
                <v-row>
...
export default class MmuCard extends Mixins(StateMixin, MmuMixin) {
    ...
    get title(): string {
        const headline = this.$t('app.mmu.title.headline') as string
        if (!this.enabled) {

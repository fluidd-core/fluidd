    ...
    >
      ...
      >
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <app-btn color="warning" text @click="saveDialog.open = false" type="button">{{ $t('app.general.btn.cancel') }}</app-btn>
...
import { BedMesh as IBedMesh } from '@/store/printer/types'
...
export default class BedMesh extends Mixins(StateMixin, ToolheadMixin) {
  ...
  onCurrentMeshChange (val: IBedMesh) {
    if (val) {
      this.applyMesh(val)
    }
  }

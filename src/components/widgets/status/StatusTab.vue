...
export default class StatusTab extends Mixins(StateMixin, FilesMixin) {
   ...
   */
  get filament_weight_total () {
    return this.$store.state.printer.printer.current_file.filament_weight_total || 0
  }
  ...
  get filament_weight_needed () {
    if (this.filament_used > 0 && this.filament_total > 0 && this.filament_weight_total > 0) {
      return (1 - this.filament_used / this.filament_total) * this.filament_weight_total
    } else {
      return 0
    }
  }

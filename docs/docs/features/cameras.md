...
- **MJPEG Adaptive**
    - Pulls images from the mjpegstream/ustreamer service using the snapshot
      URL.
    - Allows setting a target FPS, enabling your browser to maintain the
      intended frame rate.
    - A more reliable approach in certain scenarios.

- **UV4L-MJPEG Stream**
    - Similar to regular MJPEG Stream, however it has lower impact on the
      browser as it doesn't use a worker to read the stream and thus has no FPS
      indication.

- **HLS Stream**
    - Loads an HLS (HTTP Live Streaming) video stream via

...
??? question "Camera feed not loading"
    ...
    - If using MJPEG with Crowsnest, confirm Crowsnest is active by checking the
      service status on the [System page](system-and-notifications.md).

??? question "High latency or choppy stream"
...
??? question "Feed shows but rotation is wrong"

...
- JWT auth over WebSocket: `runIdentify` sends `server.connection.identify` with the stored user token (refreshed proactively if expired); if both tokens are expired, identify is called without a token (anonymous/trusted identify). On success it loads every Moonraker DB namespace Fluidd owns (skipping any with empty ROOTS) and the config file list before transitioning to `ready`; on failure → `authenticating`. `server.connection.identify` is one-shot per socket, so the logout→login path (same physical socket, new user via `access.login`) skips the identify call (connectionId already set) and loads databases directly before transitioning to `ready`
...
- On entry to `ready`, `runBootstrap` fires off `server.info` and config file list refreshes (fire-and-forget; data flows through `onServerRead` handler) — the main app is now safe to render

...
- On entry to `ready`, `onSetStatus` fires off `server.info` and config file list refreshes (fire-and-forget; data flows through the socket's `onServerRead` handler) — the main app is now safe to render
